...
# Fix for RpcError import - try multiple possible locations
try:
    ...
    try:
        ...
        try:
            from telethon.errors.common import RPCError as RpcError
        except ImportError:
            # Fallback - create a basic exception class if none found
    ...
    async def transcribe_handler(event):
        ...
        try:
            ...
            try:
                ...
                # Catch any other Telegram-related errors
                if "PREMIUM" in str(e).upper() or "USER_PREMIUM_REQUIRED" in str(e):
                    await event.reply("⚠ Premium required. Using free API...")
                else:
                    await event.reply(f"⚠ Telegram error: `{e}`\nUsing free API...")
