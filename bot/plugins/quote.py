...
def sanitize_name(name):
    """
    Sanitize name by converting special fonts/characters to normal text
    and removing emojis and other non-text characters
    """
    if not name:
        return "User"
    
    # First normalize unicode characters (converts special fonts to normal)
    ...
    # Return "User" if name becomes empty after sanitization
            ...
            if query and query.strip().lower() == 'r':
                ...
                if message_b_user:
                    if hasattr(message_b_user, 'first_name') and message_b_user.first_name:
                        message_b_username = sanitize_name(message_b_user.first_name)
                    elif hasattr(message_b_user, 'title') and message_b_user.title:
                        message_b_username = sanitize_name(message_b_user.title)
                ...
                if message_a_user:
                    if hasattr(message_a_user, 'first_name') and message_a_user.first_name:
                        message_a_username = sanitize_name(message_a_user.first_name)
                    elif hasattr(message_a_user, 'title') and message_a_user.title:
                        message_a_username = sanitize_name(message_a_user.title)
                ...
                # Get the message author's name (if replying) or use command user as fallback
                username = "User"
                if event.is_reply:
                    reply_msg = await event.get_reply_message()
                    if reply_msg and reply_msg.sender:
                        reply_user = reply_msg.sender
                        if hasattr(reply_user, 'first_name') and reply_user.first_name:
                            username = sanitize_name(reply_user.first_name)
                        elif hasattr(reply_user, 'title') and reply_user.title:
                            username = sanitize_name(reply_user.title)
                else:
                    # Fallback to command user if not replying
                    user = await client.get_me()
                    if user.first_name:
                        username = sanitize_name(user.first_name)
